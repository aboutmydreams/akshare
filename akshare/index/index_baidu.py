...
def baidu_search_index(
    ...
    if province == "全国":
        ...
        if result_list := [
            item
            for item in baidu_area_map
            if item[0] == province and item[1] == city
        ]:
...
def baidu_info_index(
    ...
    with session.get(url="http://index.baidu.com/api/FeedSearchApi/getFeedIndex", params=params) as response:
        data = response.json()["data"]
...
def baidu_media_index(
    ...
    with session.get(url="http://index.baidu.com/api/NewsApi/getNewsIndex", params=params) as response:
        data = response.json()["data"]
        all_data = data["index"][0]["data"]
        uniqid = data["uniqid"]
        ptbk = get_ptbk(uniqid, cookie)
